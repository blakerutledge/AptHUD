...
var app = express(); //Gzip compression

app.use(compression()); // Allow CORS
...
app.set('view engine', 'dot'); // Routes

app.use('/', index); // favicon

app.use(favicon(path.join(__dirname, '..', 'dist', 'public', 'favicon.ico'))); // Webhook debounced, wont fetch more than 1 every 3sec
// Successive webhooks restart the timer

var contentfulDebouncing = false;
var contentfulTimeout;
...
app.use(express["static"](path.join(__dirname, '..', 'dist', 'public'))); // catch 404 and forward to error handler

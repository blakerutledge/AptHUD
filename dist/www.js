...
var config = require(path.join(__dirname, '..', 'config.json')); // Create HTTP server
...
http.on('listening', onListening); // Start application with SocketIO

var socket = require('./socket').open(io); // Error starting server at specified port 
...
function watchFrontend() {
  chokidar.watch(path.join(__dirname, '..', 'dist', 'public'), {
    ignored: /(^|[\/\\])\../
